// Persistent storage for RABKL articles using multiple fallback strategies
// This ensures articles persist across serverless function executions

// Fallback storage strategies in order of preference:
// 1. Environment variables (for small datasets)
// 2. File system with temp directory
// 3. In-memory with cross-function sharing

let articlesCache = [];
const STORAGE_KEY = 'rabkl_articles';
const MAX_ENV_SIZE = 32000; // Conservative limit for environment variable storage

// Initialize storage with sample articles
...
async function loadArticles() {
  try {
    // Strategy 1: Try to load from file system
    const fs = await import('fs').catch(() => null);
    const path = await import('path').catch(() => null);
    
    if (fs && path) {
      const tmpDir = '/tmp';
      const filePath = path.join(tmpDir, 'rabkl-articles.json');
      
      try {
        if (fs.existsSync(filePath)) {
          const data = fs.readFileSync(filePath, 'utf8');
          const articles = JSON.parse(data);
          console.log(`Loaded ${articles.length} articles from file system`);
          return articles;
        }
      } catch (fileError) {
        console.log('File system storage not available:', fileError.message);
      }
    }
    
    // Strategy 2: Try to load from environment variable
    const envData = process.env.RABKL_ARTICLES_DATA;
    if (envData) {
      try {
        const articles = JSON.parse(envData);
        console.log(`Loaded ${articles.length} articles from environment`);
        return articles;
      } catch (envError) {
        console.log('Environment storage parse error:', envError.message);
      }
    }
    
    console.log('No persistent storage found, using in-memory cache');
    return [];
...
async function saveArticles(articles) {
  try {
    ...
    // Strategy 1: Try to save to file system
    const fs = await import('fs').catch(() => null);
    const path = await import('path').catch(() => null);
    ...
    if (fs && path) {
      ...
      try {
        ...
        fs.writeFileSync(filePath, articlesJson, 'utf8');
        console.log(`Saved ${articles.length} articles to file system`);
        return true;
      } catch (fileError) {
        console.log('File system save failed:', fileError.message);
      }
    }
    
    // Strategy 2: Try to save to environment variable (for small datasets)
    if (articlesJson.length < MAX_ENV_SIZE) {
      try {
        process.env.RABKL_ARTICLES_DATA = articlesJson;
        console.log(`Saved ${articles.length} articles to environment`);
        return true;
      } catch (envError) {
        console.log('Environment save failed:', envError.message);
      }
    } else {
      console.log('Articles data too large for environment storage');
    }
...
// Get all articles with persistent loading
export async function getArticles() {
  // If cache is empty, try to load from persistent storage
  if (articlesCache.length === 0) {
    const persistedArticles = await loadArticles();
    if (persistedArticles.length > 0) {
      articlesCache = persistedArticles;
    } else {
      // Initialize with sample articles if no persisted data
...
// Add article with persistent saving
export async function addArticle(article) {
  try {
    // Ensure we have the latest articles
    await getArticles();
    ...
    await saveArticles(articlesCache);
...
export async function initializeSampleArticlesIfNeeded() {
  const articles = await getArticles();
